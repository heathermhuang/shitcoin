...
.wrap{position:relative;z-index:1;max-width:1700px;margin:0 auto;padding:32px 20px}
.mono{font-family:'IBM Plex Mono',monospace}

/* HEADER */
.hdr{display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:16px;margin-bottom:28px}
.hdr-left{display:flex;align-items:center;gap:14px}
.logo{width:42px;height:42px;background:linear-gradient(135deg,var(--red),var(--amber));border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:800;color:#000;font-family:'IBM Plex Mono',monospace;box-shadow:0 0 24px rgba(239,68,68,0.2)}
...
/* STATS */
.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:10px;margin-bottom:24px}
.stat{background:var(--bg-2);border:1px solid var(--border-1);border-radius:10px;padding:16px}
.stat-label{font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:var(--text-3);font-family:'IBM Plex Mono',monospace;margin-bottom:6px}
...
.stat-sub{font-size:10px;color:var(--text-3);font-family:'IBM Plex Mono',monospace;margin-top:2px}
...
/* Loading */
.loading-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:var(--bg-0);display:flex;align-items:center;justify-content:center;z-index:100;transition:opacity 0.3s}
.loading-overlay.hidden{opacity:0;pointer-events:none;visibility:hidden}
.loader{text-align:center}
.loader .spinner{width:32px;height:32px;border:3px solid var(--border-2);border-top-color:var(--amber);border-radius:50%;animation:spin 0.8s linear infinite;margin:0 auto 12px}
.loader .ltxt{font-family:'IBM Plex Mono',monospace;font-size:12px;color:var(--text-3)}
...
/* EXCHANGE SWITCHER */
.ex-switcher{display:flex;align-items:center;justify-content:center;gap:4px;padding:20px 20px 0;position:relative;z-index:2}
.ex-tab{padding:10px 28px;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer;transition:all 0.2s;border:1px solid var(--border-1);background:var(--bg-2);color:var(--text-3);font-family:'DM Sans',sans-serif;display:flex;align-items:center;gap:8px;letter-spacing:-0.3px}
.ex-tab:hover{background:var(--bg-3);color:var(--text-2);border-color:var(--border-2)}
.ex-tab.active{color:#fff;border-color:transparent}
.ex-tab.active.binance{background:linear-gradient(135deg,#F0B90B,#d4a20a);color:#000;box-shadow:0 0 20px rgba(240,185,11,0.25)}
.ex-tab.active.coinbase{background:linear-gradient(135deg,#0052FF,#3d7bff);color:#fff;box-shadow:0 0 20px rgba(0,82,255,0.25)}
.ex-tab-icon{width:20px;height:20px;border-radius:4px;display:flex;align-items:center;justify-content:center;font-family:'IBM Plex Mono',monospace;font-size:11px;font-weight:800}
...
<div class="ex-switcher">
    <button class="ex-tab binance active" id="ex-tab-binance" onclick="switchExchange('binance')">
        <span class="ex-tab-icon">₿</span> Binance
    </button>
    <button class="ex-tab coinbase" id="ex-tab-coinbase" onclick="switchExchange('coinbase')">
        <span class="ex-tab-icon">CB</span> Coinbase
    </button>
</div>

<!-- BINANCE SECTION -->
<div class="ex-section active" id="bn-section">
<div class="loading-overlay" id="bn-loader"><div class="loader"><div class="spinner"></div><div class="ltxt">Loading live data from Binance...</div></div></div>
<div class="wrap">
<div class="hdr">
    <div class="hdr-left">
        <div class="logo">⚡</div>
        <div><h1>Binance Monitor</h1><div class="sub mono">risk scores · monitoring tags · delistings · all coins</div></div>
    </div>
    <div class="hdr-right">
...
<div class="loading-overlay" id="cb-loader" style="display:none"><div class="loader"><div class="spinner"></div><div class="ltxt">Loading live data from Coinbase...</div></div></div>
<div class="wrap">
<div class="hdr">
    <div class="hdr-left">
        <div class="logo">CB</div>
        <div><h1>Coinbase Monitor</h1><div class="sub mono">risk scores · trading status · delistings · all coins</div></div>
    </div>
    <div class="hdr-right">
        <div class="live"><span class="live-dot"></span>LIVE</div>
...
<div class="stats" id="cb-statsBar">
    <div class="stat"><div class="stat-label">Online</div><div class="stat-val g" id="cb-sOnline">-</div></div>
    <div class="stat"><div class="stat-label">Limit Only</div><div class="stat-val a" id="cb-sLimit">-</div></div>
    <div class="stat"><div class="stat-label">Delisted</div><div class="stat-val r" id="cb-sDelisted">-</div></div>
    <div class="stat"><div class="stat-label">Total Tracked</div><div class="stat-val b" id="cb-sTotal">-</div></div>
    <div class="stat"><div class="stat-label">High Risk</div><div class="stat-val r" id="cb-sHighRisk">-</div></div>
    <div class="stat"><div class="stat-label">Active Pairs</div><div class="stat-val p" id="cb-sPairs">-</div></div>
</div>
<div class="filters">
...
async function fetchAllData() {
    // Step 0: Fetch exchangeInfo to know which pairs are actually TRADING
    let tradingPairs = null;
    try {
        const eiResp = await fetch(BINANCE_API + '/exchangeInfo');
        if (eiResp.ok) {
            const ei = await eiResp.json();
            tradingPairs = new Set();
            ei.symbols.forEach(s => {
                if (s.quoteAsset === 'USDT' && s.status === 'TRADING') {
                    tradingPairs.add(s.baseAsset);
                }
            });
    ...
    // Step 1: Fetch all 24hr tickers (this also discovers all active coins)
    let tickerOk = false;
    try {
        const resp = await fetch(BINANCE_API + '/ticker/24hr');
        if (!resp.ok) throw new Error('Ticker HTTP ' + resp.status);
        const all = await resp.json();
        if (!Array.isArray(all) || all.length === 0) throw new Error('Empty ticker response');
...
async function fetchMarketCaps() {
    try {
        ...
        // Build symbol->coingecko_id map
        const cgResp = await fetch('/cg/coins/list');
        if (!cgResp.ok) return;
        const cgList = await cgResp.json();
        const cgBySym = {};
        cgList.forEach(c => {
            const s = c.symbol.toUpperCase();
            if (!cgBySym[s]) cgBySym[s] = c.id;
        });
        ...
        const getGeckoId = sym => overrides[sym] || cgBySym[sym] || sym.toLowerCase();
        ...
        for (const batch of batches) {
            ...
            await new Promise(r => setTimeout(r, 1500)); // CoinGecko rate limit
        }
...
async function fetchDepthData() {
    ...
    const candidates = allCoins.filter(t => {
        if (t.status === 'monitoring' || t.status === 'delisting' || t.status === 'restored') return true;
        if (t.status === 'active' && t._risk >= 30) return true;
        return false;
    }).filter(t => liveData[t.sym]?.price);
...
async function fetchCoinGeckoData() {
    try {
        // Get CoinGecko coin list for ID lookup
        const listResp = await fetch('/cg/coins/list');
        if (!listResp.ok) return;
        const cgList = await listResp.json();
        const cgBySym = {};
        cgList.forEach(c => { const s = c.symbol.toUpperCase(); if (!cgBySym[s]) cgBySym[s] = c.id; });
        const getGeckoId = sym => CG_OVERRIDES[sym] || cgBySym[sym] || sym.toLowerCase();
