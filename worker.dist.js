...
/* EXCHANGE SWITCHER */
.ex-switcher{display:flex;align-items:center;justify-content:center;gap:0;padding:24px 20px 0;position:relative;z-index:2}
.ex-switcher-inner{display:flex;background:var(--bg-2);border:1px solid var(--border-1);border-radius:10px;padding:3px;gap:3px}
.ex-tab{padding:9px 24px;border-radius:7px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;border:1px solid transparent;background:transparent;color:var(--text-3);font-family:'DM Sans',sans-serif;display:flex;align-items:center;gap:7px;letter-spacing:-0.2px}
.ex-tab:hover{color:var(--text-2)}
.ex-tab.active{color:#fff;border-color:transparent}
.ex-tab.active.binance{background:linear-gradient(135deg,#F0B90B,#c89209);color:#000;box-shadow:0 2px 12px rgba(240,185,11,0.3)}
.ex-tab.active.coinbase{background:linear-gradient(135deg,#0052FF,#2d6fff);color:#fff;box-shadow:0 2px 12px rgba(0,82,255,0.3)}
.ex-tab-icon{font-size:14px;font-family:'IBM Plex Mono',monospace;font-weight:800}
...
async function fetchAllData() {
    ...
    setTimeout(() => document.getElementById('bn-loader').classList.add('hidden'), 200);
